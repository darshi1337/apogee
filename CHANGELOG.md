...
- **Bluesky post extractor.** Dedicated extractor for Bluesky (`bsky.app`) post pages that fetches the thread via the public AT Protocol endpoint `https://public.api.bsky.app/xrpc/app.bsky.feed.getPostThread` without authentication and falls back to DOM parsing when offline, parsing post authors, like counts, and reply chains up to 80 posts and depth 8 into structured Markdown. (#147)
