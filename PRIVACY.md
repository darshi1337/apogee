...
- **Bluesky Thread Fetch**: On a Bluesky (`bsky.app`) post page, the extractor fetches the thread from the public AT Protocol endpoint `https://public.api.bsky.app/xrpc/app.bsky.feed.getPostThread` to get the fully nested reply tree without auth. Like the YouTube transcript fetch, this runs in the content script and is gated by `optional_host_permissions` (`*://*.bsky.app/*`, which covers `public.api.bsky.app`, asked on demand) and `activeTab`. The request carries only the public `at://` URI of the post (taken from the URL you already view) and falls back to local DOM parsing when offline, blocked, or permission is denied, with no cookies or page content beyond the post ID.
- **Reddit Thread JSON (same-origin page context)**: On a Reddit comments page, the content-script extractor fetches that page own thread JSON (`{page origin}{page path}.json`, same origin as the comments page you already open) to get the fully nested reply tree without auth. Like the YouTube transcript fetch, this runs in the content script in the page own context, so it reaches no host other than the Reddit page itself and is gated by `activeTab` instead of by `optional_host_permissions` or the extension-pages `connect-src` list below. The request sends no cookies (`credentials: "omit"`), carries no page content beyond the URL already viewed, and returns `null` (falling back to the generic extractor) when offline or blocked.
...
- **Enforced Connection Allow-List**: No other outside calls exist. The `connect-src` in `apogee-extension/manifest.json` is the exact allow-list enforced against extension pages (popup, background, offscreen). Content-script fetches (YouTube transcripts and the Reddit same-origin thread JSON above) run in the page own context instead, so they are bound by the extractor host allow-list plus `optional_host_permissions`/`activeTab`, as stated per item. `validateLoopbackUrl` in `apogee-extension/lib/util/ollamaHost.js` is the single shared loopback validator for the settings UI, the service worker, and the diagnostics display: it limits local links only to loopback hosts (`http://127.0.0.1` or `http://localhost`, IPv6 loopback rejected) with strict port checks (default port 11434 for Ollama, 8080 for llama.cpp).
...
DOCX files picked or dropped into the popup are parsed locally from their ZIP/XML structure without a document-conversion service or heavy parser dep. Text, Markdown, JSON, and HTML files, plus pasted text, are also read locally. Picked files are capped at 50 MB and pasted or plain-text input at 100,000 characters, with a note when truncation happens. These inputs go only to the picked on-device engine or the clearly set loopback Ollama/llama.cpp server.
