...
Videos (YouTube and Bilibili) get their own treatment: Apogee pulls the video's timestamped transcript and produces a short written gist plus a "Key moments" timeline, sized to the video's length, where every entry is a clickable link that seeks the video to that moment. When a video's description defines real chapters, the summary follows those chapters instead. Sponsor reads and self-promotion are stripped from YouTube transcripts first (see the SponsorBlock lookup in PRIVACY.md).
...
- **Loopback Origin Handling**: The `Origin` header is stripped from the extension's own `localhost`/`127.0.0.1` requests (Ollama and llama.cpp) so local servers accept them with no extra setup. Where session-scoped rules are supported this is registered at runtime for non-tab requests only, so site pages keep their `Origin` headers; the bundled static `rules/ollama-cors.json` remains as a fallback.
- **SponsorBlock Gating**: The SponsorBlock segment lookup runs only when `useSponsorBlock` is enabled and the optional host permission is granted. With "Stay fully local" on, no lookup request is made and a local phrase heuristic strips sponsor reads instead.
